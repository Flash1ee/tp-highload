...
Перекодирование голоса и видео - требовательная операция к CPU, поэтому необходимо большое кол-во ядер.
...
Общая сетевая нагрузка от потоков: 2700 * 2 = 5400 ГБит/с

Обеспечим запас в 25%: 5400 * 0.25 = 1350 ГБит/с
...
Нам потребуется 121462.5 / 40 = **169 серверов\***
...
Пиковая нагрузка - 322 ГБит/с

Сетевой трафик Пиковая нагрузка = 322 * 0.25 = 80,5 Гбит/с

80,5 Гбит/c / 2 регионов = 40,25 Гбит/с на регион 
...
В каждом регионе по 20 сервера, подходящий сервер будет выбираться DNS балансером.

На регион по 20 серверов.  
